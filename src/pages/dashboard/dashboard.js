...
class Dashboard extends Component {
	...
	componentWillMount() {
		...
		if (login_datails.admin_role !== 0) {
			this.setState({ admin_role: login_datails.admin_role });
		}
		dashBaord()
			.then((data) => {
		...
		return (
			<Container fluid className='main-content-container px-4'>
				...
				<Row>
					...
					{(this.state.admin_role === 0 || this.state.admin_role === 2) && (
					...
					{(this.state.admin_role === 0 || this.state.admin_role === 1) && (
						<Col className='col-lg mb-4'>
							<Card className='bg-danger' small>
								<Link to='sub-admin'>
									<div style={this.style.count}>
									...
									<div style={this.style.text}>{this.state.subAdmins}</div>
								</Link>
							</Card>
						</Col>
					)}
