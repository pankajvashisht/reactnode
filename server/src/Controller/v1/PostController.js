...
module.exports = {
    getPost: async (req, res) => {
        ...
        try {
            ...
            query+="(select count(*) from users_posts where user_id = "+request_data.user_id+" and post_id = posts.id) as is_buy from posts where status = 1"; 
    ...
    myposts: async (req, res) => {
        ...
        try {
            ...
            query+= "(select count(*) from users_posts where user_id = "+request_data.user_id+" and post_id = posts.id) as is_buy from users_posts" 
            query+= " join posts on (posts.id = users_posts.post_id)"
            query+= " where posts.status = 1 and users_posts.status =1" 
            query+= " order by posts.id desc limit "+offset+" , 20";
            let result = await DB.first(query);
            result.forEach((value, key) => {
...
const postDetails = async function(id){
    ...
    query+= "posts join users_posts on (posts.id = users_posts.post_id) where posts.id = "+id+" limit 1";
    let result = await DB.first(query);
    result.forEach((value, key) => {
        result[key].url = app.ImageUrl(value.url);
    });
