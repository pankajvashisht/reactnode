...
module.exports = {
    ...
    const MailConfig = require('../config/mails');
		...
		try {
			var transporter = nodemailer.createTransport(MailConfig[MailConfig.default]);
	...
	stripe:async function(){
		...
		try{
			if(fs.existsSync(config.root_path+'model/'+file_name+'.js')){
				...
				let message="Model "+file_name+" Not Found on the server.Please create the "+file_name+" in model folder.";
